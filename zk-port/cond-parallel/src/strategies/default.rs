...
impl DefaultThreadManager {
    pub(crate) fn new() -> DefaultThreadManager {
        ...
        let exe_threads = thread_pool(
            "exe".into(),
            Some(min(num_cpus::get(), MAX_THREAD_POOL_SIZE)),
        );
        let non_exe_threads = thread_pool(
            "non_exe".into(),
            Some(min(num_cpus::get(), MAX_THREAD_POOL_SIZE)),
        );
        let io_threads = thread_pool("io".into(), Some(64));
        let background_threads =
            thread_pool("background".into(), Some(MAX_THREAD_POOL_SIZE));
        Self {
            exe_threads,
            non_exe_threads,
            io_threads,
