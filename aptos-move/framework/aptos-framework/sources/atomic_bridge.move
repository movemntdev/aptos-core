...
module aptos_framework::ethereum {
    ...
    public fun to_eip55_checksumed_address(ethereum_address: &vector<u8>): vector<u8> {
        ...
        assert!(is_hex_digit_vector(ethereum_address), ENOT_EIP55_ADDRESS);
    ...
    public fun is_eip55_compliant(ethereum_address: &vector<u8>): bool {
        assert!(vector::length(ethereum_address) == ETH_ADDRESS_LENGTH, ENOT_ETH_ADDRESS_LENGTH);
        assert!(is_hex_digit_vector(ethereum_address), ENOT_EIP55_ADDRESS);
    ...
    #[test]
    fun foo() {
        let a = b"32Bh343B94f860124dC4fEe278FDCBD38C102D88";
...
module aptos_framework::atomic_bridge_initiator {
    ...
    public entry fun initiate_bridge_transfer(
        ...
        assert!(
            vector::length(&recipient) == ethereum::ethereum_address_length(),
            ENOT_ETH_ADDRESS_LENGTH
        );
    ...
    public entry fun complete_bridge_transfer(
        ...
        let (_, _) =
            atomic_bridge_store::complete_transfer<address, EthereumAddress>(
                bridge_transfer_id, create_hashlock(pre_image)
            );

        event::emit(
            BridgeTransferCompletedEvent { bridge_transfer_id, pre_image }
        );
    }

    /// Anyone can refund the transfer on the source chain once time lock has passed
    public entry fun refund_bridge_transfer(
        _caller: &signer, bridge_transfer_id: vector<u8>
    ) {
        let (receiver, amount) =
            atomic_bridge_store::cancel_transfer<address, EthereumAddress>(
                bridge_transfer_id
            );
        atomic_bridge::mint(receiver, amount);

        event::emit(
            BridgeTransferRefundedEvent { bridge_transfer_id }
        );
    }
    ...
    fun test_initiate_bridge_transfer(
        ...
        let bridge_transfer_initiated_event = vector::borrow(
            &event::emitted_events<BridgeTransferInitiatedEvent>(), 0
        );
        assert_valid_bridge_transfer_id(
            &bridge_transfer_initiated_event.bridge_transfer_id
        );
    ...
    fun test_complete_bridge_transfer(
        ...
        let bridge_transfer_id = vector::borrow(
            &event::emitted_events<BridgeTransferInitiatedEvent>(), 0
        ).bridge_transfer_id;
    ...
    #[test(aptos_framework = @aptos_framework, sender = @0xdaff)]
    #[expected_failure(abort_code = 0x1, location = 0x1::atomic_bridge_store)]
    // EINVALID_PRE_IMAGE
    ...
    #[test(aptos_framework = @aptos_framework, sender = @0xdaff)]
    #[expected_failure(abort_code = 0x10001, location = 0x1::smart_table)]
    // ENOT_FOUND
    ...
    fun test_refund_bridge_transfer(
        ...
        timestamp::fast_forward_seconds(
            atomic_bridge_configuration::initiator_timelock_duration() + 1
        );
    ...
    #[test(aptos_framework = @aptos_framework, sender = @0xdaff)]
    #[expected_failure(abort_code = 0x4, location = 0x1::atomic_bridge_store)]
    //ENOT_EXPIRED
...
module aptos_framework::atomic_bridge_store {
    ...
    public fun initialize(aptos_framework: &signer) {
        ...
        move_to(aptos_framework, Nonce { inner: 0 });
        ...
        let counterparties = SmartTableWrapper<vector<u8>, BridgeTransferDetails<
            EthereumAddress,
            address>> { inner: smart_table::new() };
    ...
    public(friend) fun create_details<Initiator: store, Recipient: store>(
        initiator: Initiator,
        recipient: Recipient,
        amount: u64,
        hash_lock: vector<u8>,
    ...
    public(friend) fun add<Initiator: store, Recipient: store>(
        bridge_transfer_id: vector<u8>, details: BridgeTransferDetails<Initiator, Recipient>
        ...
        let table =
            borrow_global_mut<SmartTableWrapper<vector<u8>, BridgeTransferDetails<Initiator, Recipient>>>(
                @aptos_framework
            );
    ...
    fun assert_pending<Initiator: store, Recipient: store>(
    ...
    fun assert_valid_hash_lock(hash_lock: &vector<u8>) {
        assert!(vector::length(hash_lock) == HASH_LOCK_LENGTH, EINVALID_HASH_LOCK);
    }
    ...
    fun assert_correct_hash_lock<Initiator: store, Recipient: store>(
        details: &BridgeTransferDetails<Initiator, Recipient>, hash_lock: vector<u8>
    ) {
    ...
    fun assert_timed_out_lock<Initiator: store, Recipient: store>(
        details: &BridgeTransferDetails<Initiator, Recipient>
    ) {
    ...
    fun assert_within_timelock<Initiator: store, Recipient: store>(
        details: &BridgeTransferDetails<Initiator, Recipient>
    ) {
    ...
    fun complete<Initiator: store, Recipient: store>(
    ...
    fun cancel<Initiator: store, Recipient: store>(
        details: &mut BridgeTransferDetails<Initiator, Recipient>
    ) {
    ...
    fun complete_details<Initiator: store, Recipient: store + copy>(
    ...
    public(friend) fun complete_transfer<Initiator: store, Recipient: copy + store>(
        bridge_transfer_id: vector<u8>, hash_lock: vector<u8>
    ): (Recipient, u64) acquires SmartTableWrapper {
        assert!(features::abort_atomic_bridge_enabled(), EATOMIC_BRIDGE_NOT_ENABLED);
    ...
    fun cancel_details<Initiator: store + copy, Recipient: store>(
        details: &mut BridgeTransferDetails<Initiator, Recipient>
    ): (Initiator, u64) {
        assert_pending(details);
    ...
    public(friend) fun cancel_transfer<Initiator: store + copy, Recipient: store>(
        bridge_transfer_id: vector<u8>
    ): (Initiator, u64) acquires SmartTableWrapper {
        assert!(features::abort_atomic_bridge_enabled(), EATOMIC_BRIDGE_NOT_ENABLED);
    ...
    public(friend) fun bridge_transfer_id<Initiator: store, Recipient: store>(
        details: &BridgeTransferDetails<Initiator, Recipient>
    ): vector<u8> acquires Nonce {
    ...
    fun get_bridge_transfer_details<Initiator: store + copy, Recipient: store + copy>(
        bridge_transfer_id: vector<u8>
    ): BridgeTransferDetails<Initiator, Recipient> acquires SmartTableWrapper {
        let table =
            borrow_global<SmartTableWrapper<vector<u8>, BridgeTransferDetails<Initiator, Recipient>>>(
                @aptos_framework
            );

        let details_ref = smart_table::borrow(
            &table.inner, bridge_transfer_id
        );
    ...
    public fun test_get_bridge_transfer_details_initiator(
    ...
    #[test(aptos_framework = @aptos_framework)]
    public fun test_get_bridge_transfer_details_counterparty(
        aptos_framework: &signer
    ) acquires SmartTableWrapper {
        timestamp::set_time_has_started_for_testing(aptos_framework);
        features::change_feature_flags_for_testing(
        ...
        let retrieved_details =
            get_bridge_transfer_details_counterparty(bridge_transfer_id);
...
module aptos_framework::atomic_bridge_configuration {
    ...
    public fun update_bridge_operator(
        aptos_framework: &signer, new_operator: address
        ...
        event::emit(
            BridgeConfigOperatorUpdated { old_operator, new_operator }
        );
    }

    public fun set_initiator_time_lock_duration(
        aptos_framework: &signer, time_lock: u64
    ) acquires BridgeConfig {
        system_addresses::assert_aptos_framework(aptos_framework);
        borrow_global_mut<BridgeConfig>(@aptos_framework).initiator_time_lock = time_lock;

        event::emit(
            InitiatorTimeLockUpdated { time_lock }
        );
    }

    public fun set_counterparty_time_lock_duration(
        aptos_framework: &signer, time_lock: u64
    ) acquires BridgeConfig {
        system_addresses::assert_aptos_framework(aptos_framework);
        ...
        event::emit(
            CounterpartyTimeLockUpdated { time_lock }
        );
    }
    ...
    public(friend) fun assert_is_caller_operator(caller: &signer) acquires BridgeConfig {
        assert!(
            borrow_global<BridgeConfig>(@aptos_framework).bridge_operator
    ...
    fun test_update_bridge_operator(
        aptos_framework: &signer, new_operator: address
    ) acquires BridgeConfig {
        initialize(aptos_framework);
    ...
    fun test_failing_update_bridge_operator(
        aptos_framework: &signer, bad: &signer, new_operator: address
    ) acquires BridgeConfig {
        initialize(aptos_framework);
    ...
    fun test_is_not_valid_operator(
        aptos_framework: &signer, bad: &signer
    ) acquires BridgeConfig {
        initialize(aptos_framework);
    ...
    fun test_not_able_to_set_initiator_time_lock(
        aptos_framework: &signer, bad: &signer
    ) acquires BridgeConfig {
        initialize(aptos_framework);
    ...
    fun test_not_able_to_set_counterparty_time_lock(
        aptos_framework: &signer, bad: &signer
    ) acquires BridgeConfig {
        initialize(aptos_framework);
...
module aptos_framework::atomic_bridge_counterparty {
    ...
    public entry fun complete_bridge_transfer(
        bridge_transfer_id: vector<u8>, pre_image: vector<u8>
    ) {
        let (recipient, amount) =
            atomic_bridge_store::complete_transfer<EthereumAddress, address>(
                bridge_transfer_id,
    ...
    public entry fun abort_bridge_transfer(
        caller: &signer, bridge_transfer_id: vector<u8>
    ) {
        atomic_bridge_configuration::assert_is_caller_operator(caller);
        ...
        event::emit(
            BridgeTransferCancelledEvent { bridge_transfer_id }
        );
    }
    ...
    fun test_abort_transfer_of_assets(aptos_framework: &signer) {
        ...
        timestamp::fast_forward_seconds(
            atomic_bridge_configuration::counterparty_timelock_duration() + 1
        );
    ...
    fun test_failing_complete_transfer_of_assets(
        aptos_framework: &signer
    ) {
        initialize_for_test(aptos_framework);
        timestamp::set_time_has_started_for_testing(aptos_framework);
